...
Java_dev_trindadedev_c2bsh_C2BSH_convert(JNIEnv* env,
                                         jobject,
                                         jstring j_c_code) {
  const cb_string c_code = (*env)->GetStringUTFChars(env, j_c_code, nullptr);
  c2bsh_converter_result* result_ptr = c2bsh_converter_convert(c_code);
  return (jlong)result_ptr;
}

JNIEXPORT jstring JNICALL
Java_dev_trindadedev_c2bsh_C2BSH_getCode(JNIEnv* env, jobject, jlong result) {
...
Java_dev_trindadedev_c2bsh_C2BSH_getIncludes(JNIEnv* env,
                                             jobject,
                                             jlong result) {
  c2bsh_converter_result* result_ptr = (c2bsh_converter_result*)result;
  jclass string_class = (*env)->FindClass(env, "java/lang/String");
  jobjectArray array = (*env)->NewObjectArray(env, result_ptr->includes_count,
                                              string_class, NULL);
...
JNIEXPORT void JNICALL Java_dev_trindadedev_c2bsh_C2BSH_close(JNIEnv* env,
                                                              jobject,
                                                              jlong result) {
  c2bsh_converter_result* result_ptr = (c2bsh_converter_result*)result;
